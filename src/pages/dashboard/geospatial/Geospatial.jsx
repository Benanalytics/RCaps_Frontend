...
import GrowthOperation from 'components/dashbaord/geospatial/GrowthOperation';
...
import LendingplanTable from 'components/dashbaord/geospatial/LendingplanTable';
...
const Geospatial = () => {
    ...
    const [lendingPlansGraphData, setLendingPlansGraphData] = useState('');
    const [complexScalingLendingAppData, setComplexScalingLendingAppData] =
        useState('');
    const [optimizeSelectiveTransformData, setOptimizeSelectiveTransformData] =
        useState('');
    const [growthOperationData, setGrowthOperationData] = useState([]);
    const [totalProject, setTotalProject] = useState('');
    const [totalUAAmount, setTotalUAAmount] = useState('');
    const [topThreeSectorAnalysisData, setTopThreeSectorAnalysisData] =
        useState('');
    ...
    const [
        combinedUAProjectInfraandOtherData,
        setCombinedUAProjectInfraandOtherData,
    ] = useState('');
    const [
        scalingForTransformationAndLendingVolumeData,
        setScalingForTransformationAndLendingVolumeData,
    ] = useState('');
    const [infastructureFundingData, setInfastructureFundingData] =
        useState('');
    const [strategicInvestmentData, setStrategicInvestmentData] = useState('');
    const [countryInvestmentSpectrumData, setCountryInvestmentSpectrumData] =
        useState('');
    const [financialLandscapeData, setFinancialLandscapeData] = useState('');
    const [sectoralInvestmentRangesData, setSectoralInvestmentRangesData] =
        useState('');
    const [regionMapData, setRegionMapData] = useState('');
    const [regionData, setRegionData] = useState('');
    const [lendingStatusData, setLendingStatusData] = useState('');
    ...
    const [sectorAllocationBreakDownData, setSectorAllocationBreakDownData] =
        useState('');
    ...
    const NavbarTabs = [
        ...
        { name: 'LENDING', href: '/geospatial', icon: BsMap, current: true },
    ];
    ...
    const handleTotalProjectAmountUAData = async () => {
        ...
        await instance
            .post(
                `DataManagement/Lending-plan/GetTotalProject&AmountUA`,
                data,
                ...
                if (response.status === 200) {
                    setTotalProject(
                        new Intl.NumberFormat('en-IN').format(
                            Math.round(response.data.totalProject)
                        )
                    );
                    setTotalUAAmount(
                        new Intl.NumberFormat('en-IN').format(
                            response.data.totalUAAmount
                        )
                    );
                    setTopThreeSectorAnalysisData(
                        response.data.topThreeSectorAnalysisData
                    );
                    setFirstSectorGraphData(
                        response.data.categoriesUAMillions.map((item) => {
                            return [
                                item.category,
                                item.firstSectorAnalysisUaMillion,
                            ];
                        })
                    );
                    setSecondSectorGraphData(
                        response.data.categoriesUAMillions.map((item) => {
                            return [
                                item.category,
                                item.secondSectorAnalysisUaMillion,
                            ];
                        })
                    );
                    setThirdSectorGraphData(
                        response.data.categoriesUAMillions.map((item) => {
                            return [
                                item.category,
                                item.thirdSectorAnalysisUaMillion,
                            ];
                        })
                    );
                    setTimeout(() => {
    ...
    const handleGrowthOperationData = async () => {
        ...
        const accessToken = auth.userInfo.accessToken;
        await instance
            .post(`DataManagement/Lending-plan/GetGrowOperationData`, data, {
                headers: { Authorization: `Bearer ${accessToken}` },
            })
            .then((response) => {
                if (response.status === 200) {
    ...
    const handleGetGeospatialAnalyticsGraphData = async () => {
        ...
        await instance
            .post(
                `DataManagement/Lending-plan/GetOperationType&CountryInvestSpectData`,
                data,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            )
            .then((response) => {
                if (response.status === 200) {
                    ...
                    setLendingPlansGraphData(response.data.lendingPlans);
                    setIOPCycleGraphData(response.data.iOPs);
                    setPipelineGraphData(response.data.pipelines);
                    setComplexScalingLendingAppData(
                        response.data.complexScalingLendingAppData
                    );
                    setOptimizeSelectiveTransformData(
                        response.data.optimizeSelectiveTransformData
                    );
                    setTimeout(() => {
                        setIsLoading(false);
                    }, 1000);
    ...
    const handleGetGeospatialAnalyticsData = async () => {
        ...
        await instance
            .post(
                `DataManagement/Lending-plan/GetGeospatialAnalyticsData`,
                data,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            )
            .then((response) => {
                if (response.status === 200) {
                    ...
                    setCombinedUAProjectInfraandOtherData(
                        response.data.combinedUAProjectData
                    );
                    setTimeout(() => {
                        setIsLoading(false);
                    }, 1000);
    ...
    const handleScalingForTransformationData = async () => {
        ...
        const accessToken = auth.userInfo.accessToken;
        await instance
            .post(`DataManagement/Lending-plan/GetLendingVolumeData`, data, {
                headers: { Authorization: `Bearer ${accessToken}` },
            })
            .then((response) => {
                if (response.status === 200) {
    ...
    const handleInfastructureFundingAndStrategicInvestmentData = async () => {
        setIsLoading(true);
        const data = {
            startDate: startDate,
            endDate: endDate,
        ...
        await instance
            .post(
                `DataManagement/Lending-plan/GetSectorFinancialInvestData`,
                data,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            )
            .then((response) => {
    ...
    const handleSectorAllocationBreakDownData = async () => {
        ...
        const accessToken = auth.userInfo.accessToken;
        await instance
            .post(`DataManagement/Lending-plan/GetSectorsUaMillions`, data, {
                headers: { Authorization: `Bearer ${accessToken}` },
            })
            .then((response) => {
                if (response.status === 200) {
    ...
    const handleCountryInvestmentSpectrumData = async () => {
        ...
        await instance
            .post(
                `DataManagement/Lending-plan/GetCountryWorkStat&FinancialAnalysisData`,
                data,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            )
            .then((response) => {
                if (response.status === 200) {
                    setCountryInvestmentSpectrumData(
                        response.data.countryWorkStatData
                    );
                    setFinancialLandscapeData(
                        response.data.countryFinancialData
                    );
                    setTimeout(() => {
                        setIsLoading(false);
                    }, 1000);
    ...
    const handleSectoralInvestmentRangesData = async () => {
        ...
        await instance
            .post(
                `DataManagement/Lending-plan/GetUAProjectOnSector&SectorRange`,
                data,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            )
            .then((response) => {
    ...
    const handleGetRegionMapData = async () => {
        ...
        const accessToken = auth.userInfo.accessToken;
        await instance
            .post(`DataManagement/Lending-plan/GetRegionMapData`, data, {
                headers: { Authorization: `Bearer ${accessToken}` },
            })
            .then((response) => {
                if (response.status === 200) {
    ...
    const handleGetRegionData = async () => {
        ...
        await instance
            .post(
                `DataManagement/Lending-plan/GetUAMillionOnRegionData`,
                data,
                { headers: { Authorization: `Bearer ${accessToken}` } }
            )
            .then((response) => {
    ...
    const handleLendingPlanData = async () => {
        setIsLoading(true);
        const data = {
            pageSize: pageSize,
        ...
        const accessToken = auth.userInfo.accessToken;
        await instance
            .post(`DataManagement/Lending-plan/GetLendingStatusData`, data, {
                headers: { Authorization: `Bearer ${accessToken}` },
            })
            .then((response) => {
                if (response.status === 200) {
                    setLendingStatusData(response.data);
                    setTimeout(() => {
                        setIsLoading(false);
                    }, 1000);
                }
    ...
    const handleFiltersData = async () => {
        ...
        const accessToken = auth.userInfo.accessToken;
        await instance
            .post(`DataManagement/Lending-plan/GetReportTypes`, data, {
                headers: { Authorization: `Bearer ${accessToken}` },
            })
            .then((response) => {
                if (response.status === 200) {
                ...
                <div className=''>
                    <div className='TabsLayout mb-6 flex flex-wrap justify-content-between border-b printHide '>
                        <ul className='ben__DashboardLinks flex items-center flex-wrap flex1 border0 '>
                            {NavbarTabs.map((item, ind) => (
                                <li key={item.name}>
                                    <NavLink
                                        to={`/dashboard${item.href}`}
                                        className={classNames(
                                            item.current ? '' : '',
                                            ind === NavbarTabs?.length - 1
                                                ? ''
                                                : '',
                    ...
                    <div className='TabsLayout mb-6 flex flex-wrap justify-content-between inner_Tabs printHide'>
                        <ul className='ben__DashboardLinks flex items-center flex-wrap flex1 border0 gap-x-3'>
                                    ...
                                    <NavLink
                                        ...
                                        className={classNames(
                                            ...
                                            ind === navigation?.length - 1
                                                ? ''
                                                : '',
                                            'pt-2 pb-2 px-6 flex gap-x-2 items-center semibold f13'
                                        )}
                                    >
                                        <item.icon
                                            className={classNames(
                    ...
                    <div className='pageTitle block bg-benThinBlue text-benBlue f16 rounded-md p-5 semibold mb-5 printHide'>
                        TOTAL: {totalProject} operations, amounting to UA{' '}
                        {totalUAAmount} million as Lending Plan
                    </div>
    ...
    />
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        {filtersReportTypeData &&
                                            filtersReportTypeData.map(
                                                (item) => (
                                                    <option value={item}>
                                                        {item}{' '}
                                                    </option>
      ...
      >
                                    ...
                                    <div class=''>
                                        ...
                                        <p class='text-benLightBlue f17 text-bold mb-1'>
                                            ...
                                            {new Intl.NumberFormat(
                                                ...
                                                portFolioData.amountUAMillion
                                            )}{' '}
                                            m{' '}
                                        </p>
                                        <p className='text-benGray f13 mb-3'>
                                            {new Intl.NumberFormat(
                                                'en-IN'
                                            ).format(
                                                Math.round(
                                                    portFolioData.numberOfProject
                                                )
                                            )}{' '}
                                            Project
                                        </p>
                                        ...
                                        <p className='f11'>
                                            <span
                                                className={`rounded-circle px-2 ${
                                                    portFolioData.portfolioProjectPercent >
                                                    0
                                                        ? 'badge-green'
                                                        : 'badge-red'
                                                } py-1 badge`}
                                ...
                                <div className='flex w-full gap-x-3 justify-between'>
                                    <div class=''>
                                        ...
                                        <p class='text-benLightBlue f17 text-bold mb-1'>
                                            ...
                                            {new Intl.NumberFormat(
                                                ...
                                                lendingAppData.amountUAMillion
                                            )}{' '}
                                            m
                                        </p>
                                        <p className='text-benGray f13 mb-3'>
                                            {new Intl.NumberFormat(
                                                ...
                                                Math.round(
                                                    lendingAppData.numberOfProject
                                                )
                                            )}{' '}
                                            Project
                                        </p>
                                    ...
                                    <div className='flex flex-col justify-between'>
                                        ...
                                        <p className='f11'>
                                            <span
                                                className={`rounded-circle px-2 ${
                                                    lendingAppData.lendingAppProjectPercent >
                                                    0
                                                        ? 'badge-green'
                                                        : 'badge-red'
                                                } py-1 badge`}
                                ...
                                <div className='flex w-full gap-x-3 justify-between'>
                                    <div class=''>
                                        ...
                                        <p class='text-benLightBlue f17 text-bold mb-1'>
                                            ...
                                            {new Intl.NumberFormat(
                                                ...
                                                lendingPldData.amountUAMillion
                                            )}{' '}
                                            m
                                        </p>
                                        <p className='text-benGray f13 mb-3'>
                                            {new Intl.NumberFormat(
                                                ...
                                                Math.round(
                                                    lendingPldData.numberOfProject
                                                )
                                            )}{' '}
                                            Project
                                        </p>
                                    ...
                                    <div className='flex flex-col justify-between'>
                                        ...
                                        <p className='f11'>
                                            <span
                                                className={`rounded-circle px-2 ${
                                                    lendingPldData.lendingPlProjectPercent >
                                                    0
                                                        ? 'badge-green'
                                                        : 'badge-red'
                                                } py-1 badge`}
                                ...
                                <div className='flex w-full gap-x-3 justify-between'>
                                    ...
                                    <div className='flex flex-col justify-between'>
                                        ...
                                        <p className='f11'>
                                            <span
                                                className={`rounded-circle px-2 ${
                                                    IOPCycleData.iOPProjectPercent >
                                                    0
                                                        ? 'badge-green'
                                                        : 'badge-red'
                                                } py-1 badge`}
                                ...
                                <div className='flex w-full gap-x-3 justify-between'>
                                    ...
                                    <div className='flex flex-col justify-between'>
                                        ...
                                        <p className='f11'>
                                            <span
                                                className={`rounded-circle px-2 ${
                                                    pipelineData.pipelineProjectPercent >
                                                    0
                                                        ? 'badge-green'
                                                        : 'badge-red'
                                                } py-1 badge`}
       ...
       <div id="pdf-content">
                ...
                <div className='flex h100 flex-col justify-between'>
                    <div className='card bg-white rounded-md card-shadow grid-margin printnotbreak'>
                        <div className='cardBody'>
                            ...
                            >
                                <option value=''>Select Lending Plan</option>
                                {filtersRegionData &&
                                    filtersRegionData
                                        ...
                                        <div className='flex1'>
                                            <div className='flex flex-wrap justify-between'>
                                                ...
                                                <p className='f14 text-benGray'>
                                                    UA{' '}
                                                    {regionData.amountUAMillion.toLocaleString()}{' '}
                                                    M
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <div class='progress'>
                                    {
                                        <ProgressBar
                                            data={
                                                regionData.uaMillionPercentage
                                            }
                                        />
                                    }
                                </div>
                        ...
                        <div className='cardBody'>
                            <div className=''>
                                <div class='cardTitle flex items-center'>
                                    <p class='f13 semibold  text-normal mb-4'>
                                        {new Intl.NumberFormat('en-IN').format(
                                            Math.round(
                                                lendingPldData.numberOfProject
                                            )
                                ...
                                <GeospatialGraph
                                    pipelineData={pipelineGraphData}
                                    iopData={iOPCycleGraphData}
                                    portfolioData={portFolioGraphData}
                                    lendingData={lendingPlansGraphData}
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div className='card bg-white rounded-md card-shadow grid-margin infrastructure_left printnotbreak'>
                    <div className='cardBody flex flex-col h100 gap-x-2'>
                        <div class='cardTitle flex items-center flex-wrap items-center justify-between'>
                            <div class='mb-4'>
                                <p class='f14 semibold text-uppercase text-normal'>
                                    Comparative View of Financing
                                </p>
                                <p class='f13 semibold text-light text-benGray'>
                                    {' '}
                                    Infrastructure vs. Other Initiatives All
                                    Fin. Sources
                                </p>
                            </div>
                            <div class='flex gap-x-3 mb-4 flex-wrap items-center mb-4'></div>
                        </div>
                        {combinedUAProjectInfraandOtherData &&
                            combinedUAProjectInfraandOtherData.map((item) => (
                                    ...
                                    <div className='cardBody funding-cardBody'>
                                        <div className='flex items-center w-full gap-x-2  justify-between'>
                                            <div className='w-25 mw80 funding-chart'>
                                                <InfastructurePieChart
                                                    progress={item.uaPercentage}
                                                />
                                            </div>
                                            ...
                                            <div className='flex1 chart-small'>
                                                <InfastructureLineChart
                                                    data={
                                                        item.categoriesNumberofProjects
                                                    }
                                                />
                                            </div>
                                        </div>
            ...
            <div className='card bg-white rounded-md card-shadow  grid-margin printnotbreak'>
                <LendingVolumesTable
                    data={scalingForTransformationAndLendingVolumeData}
                />
            </div>
                    ...
                    <div className='cardBody'>
                        <div class='cardTitle flex justify-between items-center mb-4 '>
                            <div className='items-self-center'>
                                <p class='f14 semibold text-uppercase text-normal'>
                                    Variable Growth In operation Sizes:
                                </p>
                                            ...
                                            <span>
                                                ...
                                                >
                                                    ...
                                                    >
                                                        ...
                                                        0 ? (
                                                            <path d='M1279 2558 l-1276 -2203 1279 -3 c703 -1 1853 -1 2556 0 l1279 3 -1276 2203 c-701 1211 -1278 2202 -1281 2202 -3 0 -580 -991 -1281 -2202z' />
                                                        ) : (
                        ...
                        <div className=''>
                            <GrowthOperation data={growthOperationData} />
                        </div>
                    </div>
                </div>
                <div className='card bg-white rounded-md card-shadow grid-margin printnotbreak'>
                    <div className='cardBody'>
                        <div class='cardTitle flex justify-between items-center mb-4 '>
                            <div className='items-self-center'>
                                ...
                                <p className='f13 text-benGray'>
                                    Highlighting Need To Advancing Beyond
                                    Micro-Scale To Larger Impact Projects
                                </p>
                            </div>
                        </div>
                        <div className=''>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 CardDivgap'>
                <div className='card bg-white rounded-md card-shadow grid-margin printnotbreak'>
                    <div className='cardBody'>
                        ...
                        <div className=''>
                            <ComplexScaling
                                data={complexScalingLendingAppData}
                            />
                        </div>
                    </div>
                </div>
                    ...
                    <div className='cardBody'>
                        <div class='cardTitle flex justify-between items-center mb-4 '>
                            <div className='items-self-center'>
                                ...
                                <p className='f13 text-benGray'>
                                    Insights into Leading Sector Investments and
                                    Projects
                                </p>
                            </div>
                        </div>
                                            ...
                                            <div className='cardBody flex cardBody15'>
                                                <div class='flex1 chart-data-text'>
                                                    ...
                                                    <div className='chart-small'>
                                                        <FirstFinanceLineChart
                                                            data={
                                                                firstSectorGraphData
                                                            }
                                                        />
                                                    </div>
                                                </div>
            ...
            <div className='flex CardDivgap SectoralInvestment_layout'>
                {/* <SectoralInvestment data={sectoralInvestmentRangesData}/> */}
                <div className='w100 card bg-white rounded-md grid-margin card-shadow infrastructure_grid printnotbreak'>
